...
const PortfolioPage = () => {
    ...
    const calculateReturn = (start, end) => {
        ...
        if (percentReturn > 0) {
          return (
            <p className="flex flex-row ml-1 mr-2 text-green-700">▲ {percentReturn.toFixed(2)}%</p>
          );
        } else {
          return (
            <p className="flex flex-row ml-1 text-red-600">▼ {percentReturn.toFixed(2)}%</p>
          );
        }
      };
  ...
  <div className="flex flex-col gap-5 my-5 ml-5 w-1/2">
    <div className="text-[#000000] bg-[#fcd46a30] hover:shadow-md rounded-2xl p-4 flex-[0.2] transition-shadow duration-300 ease-in-out text-left items-center">
      ...
      <div className="flex flex-row gap-2 ml-2 items-center">
        ...
        <p className="ml-0.5 flex flex-row items-center">({calculateReturn(balanceMonthStart, balance)} last {timeInterval})</p>
      </div>
